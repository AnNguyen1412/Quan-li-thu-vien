...
namespace LibraryManagement
{
    public partial class LibMng : Form
    {
        ...
        {
            //xoa ki tu trong tab muon tra
            txtBrwID.Text = "";
            txtBookID2.Text = "";
            txtName2.Text = "";
            txtBrwDate.Text = "";
            txtRtnDate.Text = "";
            //xoa ki tu trong tab quan li
            txtBookID.Text = "";
            txtName.Text = "";
            txtAuthor.Text = "";
            txtQuantity.Text="";
        ...
        private void btnDeleteBook_Click(object sender, EventArgs e)
        {
            if (!(txtBookID.Text == string.Empty))
            {
                ...
                {
                    MessageBox.Show("BookID accepts number only");
                    return;
                }
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            using (SqlConnection libData = new SqlConnection(cnn))
            {
                ...
                if (dt.Rows[0][0].ToString() == "1")
                {
                    ...
                    {
                        MessageBox.Show("Quantity accepts number only");
                        return;
                    }
                ...
                using (SqlConnection libData = new SqlConnection(cnn))
                {
                    ...
                    if (dt.Rows[0][0].ToString() != "0")
                    {
                        try
                        {
                            ...
                            if (String.IsNullOrWhiteSpace(txtBrwDate.Text) && String.IsNullOrWhiteSpace(txtRtnDate.Text))
                            {
                            ...
                            else if (!String.IsNullOrWhiteSpace(txtBrwDate.Text) && String.IsNullOrWhiteSpace(txtRtnDate.Text))
                            {
                    ...
                    {
                        MessageBox.Show("Borrow Unsuccessfully, please check BookID and Quantity of the book you are asking for", "Library Information");
                    }
                }
            }
            else
            {
                MessageBox.Show("Please enter BorrowID", "Library Information");
            }
        }
        private void btnRtn_Click(object sender, EventArgs e)
        {
            ...
            {
                SqlConnection connect = new SqlConnection(cnn);
                SqlCommand cmd_Brw = new SqlCommand("delete from BrwData where BorrowID ='" + this.txtBrwID.Text + "'", connect);
                SqlCommand cmd_update = new SqlCommand("UPDATE LibData SET Quantity = Quantity + 1 WHERE BookId IN (SELECT BookID FROM BrwData WHERE BorrowID ='" + this.txtBrwID.Text + "')", connect); 
                SqlDataAdapter dataAdapter = new SqlDataAdapter("select Count(*) from BrwData where BorrowID ='" + this.txtBrwID.Text + "'", connect);
                DataTable dt = new DataTable();
                try
                {
                    dataAdapter.Fill(dt);
                ...
                {
                    MessageBox.Show("BorrowID only accepts number only");
                    return;
                }
                if (dt.Rows[0][0].ToString() != "0")
                {
                    try
                    {
                        connect.Open();
                        cmd_update.ExecuteNonQuery();
                        cmd_Brw.ExecuteNonQuery();
                        MessageBox.Show("Return Successfully", "Library Infomation");
                        clearTxt();
                        connect.Close();
                    }
                    catch (Exception ec)
                ...
                {
                    MessageBox.Show("Wrong BorrowID, please check your BorrowID", "Library Infomation");
                }
            }
            else
            {
